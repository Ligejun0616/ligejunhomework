...
#define MAX(a,b) ((a) > (b) ? (a) : (b))//宏定义取出最大值
#define MIN(a,b) ((a) < (b) ? (a) : (b))//宏定义取出最小值

#define HHI(xxx) ( (byte) ((doubleword) (xxx) >> 24 )) //宏定义取出最高8位
#define LHI(xxx) ( (byte) ((doubleword) (xxx) >> 16 ) & 255 ) //宏定义取出次高8位
#define HLO(xxx) ( (byte) ((doubleword) (xxx) >> 8 ) & 255 ) //宏定义取出次低8位
#define LLO(xxx) ( (byte) ((doubleword) (xxx) & 255 )) //宏定义取出最低8位
...
qDebug()<<"最高8位:"<< pr(HHI(number)) <<dec<<HHI(number); //输出最高8位，转换为10进制
qDebug()<<"次高8位:"<< pr(LHI(number)) <<dec<<LHI(number); //输出次高8位，转换为10进制
qDebug()<<"次低8位:"<< pr(HLO(number)) <<dec<<HLO(number); //输出次低8位，转换为10进制
qDebug()<< "最低8位:"<<pr(LLO(number)) <<dec<<LLO(number)<<endl; //输出最低8位，转换为10进制
...
qDebug() <<"合成为新的32位无符号整型数为:"<<pr(i)<<i<<endl;
